...
import pytest
import numpy as np
from filters.fir_filter_lowpass import lowpass_fir_filter_firwin, lowpass_fir_filter_manual,lowpass_fir_filter_opt_manual, FilterError as FilterErrorLp
from filters.fir_filter_highpass import highpass_fir_filter_manual, highpass_fir_filter_firwin, highpass_fir_filter_opt_manual, FilterError as FilterErrorHp
from filters.fir_filter_bandpass import bandpass_fir_filter_manual, bandpass_fir_filter_firwin, bandpass_fir_filter_opt_manual, FilterError as FilterErrorBp
...
#Tests for optimized FIR lowpass filter

def test_lowpass_fir_filter_manual_opt_valid():
    """
    Test optimized lowpass FIR filter with valid parameters.
    """
    cutoff_freq = 0.3
    num_taps = 51
    coeffs = lowpass_fir_filter_opt_manual(cutoff_freq, num_taps)
    assert isinstance(coeffs, np.ndarray)
    assert len(coeffs) == num_taps

def test_lowpass_fir_filter_manual_opt_invalid_cutoff():
    """
    Test optimized lowpass FIR filter with invalid cutoff frequency.
    """
    with pytest.raises(FilterErrorLp):
        lowpass_fir_filter_opt_manual(cutoff_freq=1.5, num_taps=51)

def test_lowpass_fir_filter_opt_manual_invalid_num_taps():
    """
    Test optimized lowpass FIR filter with invalid number of taps.
    """
    with pytest.raises(FilterErrorLp):
        lowpass_fir_filter_opt_manual(cutoff_freq=0.3, num_taps=-10)

def test_lowpass_fir_filter_opt_manual_zero_cutoff():
    """
    Test lowpass FIR filter with zero cutoff frequency.
    """
    with pytest.raises(FilterErrorLp):
        lowpass_fir_filter_opt_manual(cutoff_freq=0, num_taps=51)

def test_lowpass_fir_filter_manual_opt_edge_cutoff():
    """
    Test optimized lowpass FIR filter with edge cutoff frequency.
    """
    num_taps = 51
    coeffs = lowpass_fir_filter_opt_manual(cutoff_freq=1, num_taps=51)
    assert isinstance(coeffs, np.ndarray)
    assert len(coeffs) == num_taps
...
#Tests for optimized highpass FIR filter

# Test for optimized highpass FIR filter coefficients comparison
def test_highpass_fir_filter_opt_coefficients():
    """
    Compare the FIR coefficients generated by the highpass_fir_filter_firwin and highpass_fir_filter_manual functions.
    """
    fir_coefficients_firwin = highpass_fir_filter_firwin(cutoff_freq=0.3, num_taps=51)
    fir_coefficients_opt_manual = highpass_fir_filter_opt_manual(cutoff_freq=0.3, num_taps=51)
    assert fir_coefficients_opt_manual == pytest.approx(fir_coefficients_firwin, rel=1e-7, abs=1e-3), "Coefficients are not equal"

# Additional tests for highpass FIR filter
def test_highpass_fir_filter_opt_manual_valid():
    """
    Test highpass FIR filter with valid parameters.
    """
    cutoff_freq = 0.3
    num_taps = 51
    coeffs = highpass_fir_filter_opt_manual(cutoff_freq, num_taps)
    assert isinstance(coeffs, np.ndarray)
    assert len(coeffs) == num_taps

def test_highpass_fir_filter_opt_manual_invalid_cutoff():
    """
    Test optimized highpass FIR filter with invalid cutoff frequency.
    """
    with pytest.raises(FilterErrorHp):
        highpass_fir_filter_opt_manual(cutoff_freq=1.5, num_taps=51)

def test_highpass_fir_filter_opt_manual_invalid_num_taps():
    """
    Test optimized highpass FIR filter with invalid number of taps.
    """
    with pytest.raises(FilterErrorHp):
        highpass_fir_filter_opt_manual(cutoff_freq=0.3, num_taps=-10)

def test_highpass_fir_filter_opt_manual_zero_cutoff():
    """
    Test optimized highpass FIR filter with zero cutoff frequency.
    """
    with pytest.raises(FilterErrorHp):
        highpass_fir_filter_opt_manual(cutoff_freq=0, num_taps=51)

def test_highpass_fir_filter_opt_manual_edge_cutoff():
    """
    Test highpass FIR filter with edge cutoff frequency.
    """
    num_taps = 51
    coeffs = highpass_fir_filter_opt_manual(cutoff_freq=1, num_taps=51)
    assert isinstance(coeffs, np.ndarray)
    assert len(coeffs) == num_taps
...
# Test for optimized bandpass FIR filter coefficients comparison
def test_bandpass_fir_filter_opt_coefficients():
    """
    Compare the FIR coefficients generated by the bandpass_fir_filter_firwin and bandpass_fir_filter_manual functions.
    """
    fir_coefficients_firwin = bandpass_fir_filter_firwin(lowcut=0.2, highcut=0.4, num_taps=51)
    fir_coefficients_opt_manual = bandpass_fir_filter_opt_manual(lowcut=0.2, highcut=0.4, num_taps=51)
    assert fir_coefficients_opt_manual == pytest.approx(fir_coefficients_firwin, rel=1e-7, abs=1e-3), "Coefficients are not equal"

# Additional tests for bandpass FIR filter
def test_bandpass_fir_filter_opt_manual_valid():
    """
    Test bandpass FIR filter with valid parameters.
    """
    lowcut = 0.2
    highcut = 0.4
    num_taps = 51
    coeffs = bandpass_fir_filter_opt_manual(lowcut, highcut, num_taps)
    assert isinstance(coeffs, np.ndarray)
    assert len(coeffs) == num_taps

def test_bandpass_fir_filter_opt_manual_invalid_lowcut():
    """
    Test bandpass FIR filter with invalid lowcut frequency.
    """
    with pytest.raises(FilterErrorBp):
        bandpass_fir_filter_opt_manual(lowcut=1.5, highcut=0.4, num_taps=51)

def test_bandpass_fir_filter_opt_manual_invalid_highcut():
    """
    Test bandpass FIR filter with invalid highcut frequency.
    """
    with pytest.raises(FilterErrorBp):
        bandpass_fir_filter_opt_manual(lowcut=0.2, highcut=1.5, num_taps=51)

def test_bandpass_fir_filter_opt_manual_invalid_num_taps():
    """
    Test bandpass FIR filter with invalid number of taps.
    """
    with pytest.raises(FilterErrorBp):
        bandpass_fir_filter_opt_manual(lowcut=0.2, highcut=0.4, num_taps=-10)

def test_bandpass_fir_filter_opt_manual_zero_lowcut():
    """
    Test bandpass FIR filter with zero lowcut frequency.
    """
    with pytest.raises(FilterErrorBp):
        bandpass_fir_filter_opt_manual(lowcut=0, highcut=0.4, num_taps=51)

def test_bandpass_fir_filter_opt_manual_edge_highcut():
    """
    Test bandpass FIR filter with edge highcut frequency.
    """
    num_taps=51
    coeffs = bandpass_fir_filter_opt_manual(lowcut=0.2, highcut=1, num_taps=51)
    assert isinstance(coeffs, np.ndarray)
    assert len(coeffs) == num_taps
...
# Test invalid inputs for optimized lowpass filter
def test_invalid_inputs_lowpass_opt():
    """
    Test invalid inputs for the optimized lowpass FIR filter.
    """
    with pytest.raises(FilterErrorLp):
        lowpass_fir_filter_opt_manual(-0.1, 51)
    with pytest.raises(FilterErrorLp):
        lowpass_fir_filter_opt_manual(1.5, 51)
    with pytest.raises(FilterErrorLp):
        lowpass_fir_filter_opt_manual(0.3, -10)
...
# Test invalid inputs for highpass filter
def test_invalid_inputs_opt_highpass():
    """
    Test invalid inputs for the highpass FIR filter.
    """
    with pytest.raises(FilterErrorHp):
        highpass_fir_filter_opt_manual(-0.1, 51)
    with pytest.raises(FilterErrorHp):
        highpass_fir_filter_opt_manual(1.5, 51)
    with pytest.raises(FilterErrorHp):
        highpass_fir_filter_opt_manual(0.3, -10)
...
# Test invalid inputs for optimized bandpass filter
def test_invalid_inputs_opt_bandpass():
    """
    Test invalid inputs for the bandpass FIR filter.
    """
    with pytest.raises(FilterErrorBp):
        bandpass_fir_filter_opt_manual(-0.1, 0.5, 51)
    with pytest.raises(FilterErrorBp):
        bandpass_fir_filter_opt_manual(0.2, 1.5, 51)
    with pytest.raises(FilterErrorBp):
        bandpass_fir_filter_opt_manual(0.5, 0.2, 51)
    with pytest.raises(FilterErrorBp):
        bandpass_fir_filter_opt_manual(0.2, 0.5, -10)
...
def test_plot_lowpass_opt_filter_responses():
    """
    Test the plotting of lowpass filter responses.
    """
    from filters.fir_filter_lowpass import plot_filter_opt_responses
    plot_filter_opt_responses(cutoff_freq=0.3, num_taps=51, sample_rate=1000)
...
def test_plot_highpass_filter_opt_responses():
    """
    Test the plotting of highpass filter responses.
    """
    from filters.fir_filter_highpass import plot_highpass_filter_opt_responses
    plot_highpass_filter_opt_responses(cutoff_freq=0.3, num_taps=51, sample_rate=1000)
...
def test_plot_bandpass_filter_opt_responses():
    """
    Test the plotting of bandpass filter responses.
    """
    from filters.fir_filter_bandpass import plot_bandpass_filter_opt_responses
    plot_bandpass_filter_opt_responses(lowcut=0.2, highcut=0.4, num_taps=51, sample_rate=1000)
...
def test_plot_lowpass_filter_opt_coefficients():
    ...
    Test the plotting of lowpass filter coefficients.
    """
    from filters.fir_filter_lowpass import plot_filter_opt_coefficients
    plot_filter_opt_coefficients(cutoff_freq=0.3, num_taps=51)
...
def test_plot_highpass_filter_opt_coefficients():
    """
    Test the plotting of optimized highpass filter coefficients.
    """
    from filters.fir_filter_highpass import plot_highpass_filter_opt_coefficients
    plot_highpass_filter_opt_coefficients(cutoff_freq=0.3, num_taps=51)
...
def test_plot_bandpass_filter_opt_coefficients():
    """
    Test the plotting of bandpass filter coefficients.
    """
    from filters.fir_filter_bandpass import plot_bandpass_filter_opt_coefficients
    plot_bandpass_filter_opt_coefficients(lowcut=0.2, highcut=0.4, num_taps=51)