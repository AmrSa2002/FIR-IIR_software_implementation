...
def butterworth_hp_manual_opt(order: int, cutoff: float, fs: float) -> tuple:
    """
    Generates Butterworth high-pass filter coefficients manually with proper normalization.
    Optimized code using vectorized operations for pole calculation.
    ...
    # Compute poles using vectorization (NumPy functions)
    k = np.arange(order)  # Generate an array from 0 to order-1
    angles = np.pi * (0.38 * k + 1) / (0.38 * order)  # Angles for poles
    poles = wc * np.exp(1j * angles)  # Poles (vectorized)

    # Filter only poles in the left complex plane
    poles = poles[np.real(poles) < 0]

    # Coefficients for the analog filter
    a_analog = np.poly(poles)
    b_analog = np.array([1.0] + [0] * order)  # Numerator for high-pass (reverse frequency response)
